...
void CPathBuilder::DepthCheck()
{
	CDimensionsDlg dlg;
	dlg.m_strHeading = "Depth Check";
	dlg.m_strVal1 = "Ramp length";
	dlg.m_strVal2 = "Ramp depth";
	dlg.m_strVal3 = "Seperation";
	dlg.m_strVal4 = "Dropdown";
	dlg.m_dia1 = 50;
	dlg.m_dia2 = 1;
	dlg.m_dia3 = 10;
	dlg.m_dia4 = 3;
	...
	double dLength = dlg.m_dia1;
	double zRamp   = dlg.m_dia2;
	double dWidth  = dlg.m_dia3;
	double zDepth  = dlg.m_dia4;
	m_strTitle.Format("Depth Check: Ramp %.3g in %.3g - Seperation %.3g  Dropdown %.3g", zRamp, dLength, dWidth, zDepth);

	const int numNodes = 10;
	CPathNode nd[numNodes];
	int i = 0;
	nd[i++].Set(0, 0, 0);			// starts at 0,0,0
	nd[i++].Set(0, 0, -zDepth);
	nd[i++].Set(0, 0, -zDepth-zRamp);
	nd[i++].Set(0, dLength, -zDepth);
	nd[i++].Set(0, dLength, -zDepth-zRamp);
	nd[i++].Set(dWidth, dLength, -zDepth-zRamp);
	nd[i++].Set(dWidth, dLength, -zDepth);
	nd[i++].Set(dWidth, 0, -zDepth-zRamp);
	nd[i++].Set(0, 0, -zDepth-zRamp);
	nd[i++].Set(0, 0, 0);
	ASSERT(i <= numNodes);
	int numPathNodes = i;
	...
	AddEndNode(nd[i++]);
	SetFeedRate(m_FRRoute);
	for (; i < numPathNodes-1; i++)
		AddEndNode(nd[i]);
	SetFeedRate(m_FRRapid);
	AddEndNode(nd[i++]);
}

void CPathBuilder::Rectangle()
{
	CDimensionsDlg dlg;
	dlg.m_strHeading = "Rectangle";
	dlg.m_strVal1 = "X length";
	dlg.m_strVal2 = "Y length";
	dlg.m_strVal3 = "Tool diameter";
	dlg.m_strVal4 = "Feed rate";
	dlg.m_dia1 = 0;
	dlg.m_dia2 = 0;
	dlg.m_dia3 = m_diaTool;
	dlg.m_dia4 = m_FRRoute;
	if (dlg.DoModal() != IDOK)
		return;

	SetToolDiameterMM(dlg.m_dia3);
	m_FRRoute = dlg.m_dia4;
	Rectangle(dlg.m_dia1, dlg.m_dia2);

	m_strTitle.Format("Rectangle %.3g x %.3g - Tool dia. %.4gmm", dlg.m_dia1, dlg.m_dia2, m_diaTool);
}
...
// parameters
	...
	m_zTopOfStock = 18.5;						// abs
	double zBaseUpper = 5;
	m_zTopDrill = m_zTopOfStock + 1;
	m_zClearance = m_zTopOfStock + 5;		// abs
	...
	HoleMillByZLevel(diaUpper1, m_zTopOfStock, zBaseUpper1, DIR_ZNEG, DIR_YPOS);
...
// parameters
	...
	// start at (162.0  125.6) if machine span = [1836.0, 851.2]
	// base screws are at (112, 125) so x = 162 is 50 from screws
	m_vtExtentMin = CVector(0, 0, 0);
	m_vtExtentMax = CVector(1600, 600, 0);

	double m_zTopOfStock = 18.5;			// abs
	double zBaseUpper = 5;
	m_zTopDrill = m_zTopOfStock + 1;
	m_zClearance = m_zTopOfStock + 10;		// abs !!!! allow for screw heads !!!!!
...
void CPathBuilder::ManifoldVacuumBoxSingle()
{
	ASSERT(m_pPath != NULL);
	SetDefaults();
	SetToolDiameterInch(1.0 / 4);		// 1/4" tool
...
// parameters
	...
	m_zTopOfStock = 25.0;						// abs
	double depthVacuumUpper = 22.0;
	double depthValveRecess = 4.0;
	double depthHoleOverShoot = 0.5;

	m_zTopDrill = m_zTopOfStock + 1;
	m_zClearance = m_zTopOfStock + 10;		// abs !!!! allow for screw heads !!!!!

	CDimensionsDlg dlg;
	dlg.m_strHeading = "Manifold Vacuum Hose and valve recess single pair";
	dlg.m_strVal1 = "Upper diameter";
	dlg.m_strVal2 = "Lower diameter";
	dlg.m_strVal3 = "Valve recess diameter";
	dlg.m_strVal4 = "Hole Distance";
	dlg.m_dia1 = 20.8;
	dlg.m_dia2 = 14.0;
	dlg.m_dia3 = 23.0;
	dlg.m_dia4 = 84.0;		// dist between holes
	dlg.m_bFinishCut = true;
	dlg.DoModal();

	double distHoles = dlg.m_dia4;
	double diaUpper1 = dlg.m_dia1;
	double diaLower1 = dlg.m_dia2;
	double zBaseUpper1 = m_zTopOfStock - depthVacuumUpper;
	double diaUpper2 = diaUpper1;
	double zBaseUpper2 = zBaseUpper1;
	if (dlg.m_bFinishCut)
	{
	...
	// vacuum holes
	HoleMillByZLevel(diaUpper1, m_zTopOfStock, zBaseUpper1, DIR_ZNEG, DIR_YPOS);
	HoleHelix(diaLower1, zBaseUpper1, -depthHoleOverShoot, DIR_YPOS, DIR_ZPOS);
	if (dlg.m_bFinishCut)
		HoleHelix(diaUpper2, zBaseUpper2, zBaseUpper2, DIR_YPOS, DIR_ZPOS);
	SetFeedRate(m_FRRapid);
	MoveToZ(m_zClearance);
	...
	// value recesses
	m_vtHoleLoc = CVector(distHoles, 0, m_zClearance);		// 84mm from vacuum hole
	MoveToXYZ(m_vtHoleLoc);
	HoleMillByZLevel(diaValve, m_zTopOfStock, m_zTopOfStock - depthValveRecess, DIR_ZNEG, DIR_ZPOS);
...
void CPathBuilder::ManifoldVacuumBoxArray()
{
	ASSERT(m_pPath != NULL);
	SetDefaults();
	SetToolDiameterInch(1.0 / 4);		// 1/4" tool
...
// parameters
	m_vtDelS = CVector(70, 0, 0);
	m_vtDelT = CVector(0, 60, 0);

	m_vtExtentMin = CVector(0, 0, 0);
	m_vtExtentMax = CVector(560, 180, 0);		// 9x4 array

	m_zTopOfStock = 25.0;						// abs
	double depthVacuumUpper = 22.0;
	double depthValveRecess = 4.0;
	double distXOffset = -28.0;		// = 20mm * 84/60  (20mm between hose and valve body)
	double depthHoleOverShoot = 0.5;
	...
	CDimensionsDlg dlg;
	dlg.m_strHeading = "Manifold Vacuum Hose and valve recess array of holes";
	dlg.m_strVal1 = "Upper diameter";
	dlg.m_strVal2 = "Lower diameter";
	dlg.m_strVal3 = "Valve recess diameter";
	dlg.m_strVal4 = "Hole Distance";
	...
	// value recesses
	double distHoles = dlg.m_dia4;
	double distYOffset = sqrt(distHoles*distHoles - distXOffset*distXOffset);
	CVector vtOffset(distXOffset, distYOffset, 0);				// 84mm from vacuum hole
	m_vtExtentMin += vtOffset;
	m_vtExtentMax += vtOffset;
	SetToStartMatrixHole();
	while (MoveToNextMatrixHole())
		HoleMillByZLevel(diaValve, m_zTopOfStock, m_zTopOfStock - depthValveRecess, DIR_ZNEG, DIR_ZPOS);
...
void CPathBuilder::HoleMatrixCurved()
{
	...
	for (;;)
	{
		g_utPosConvert.GetPosServoFromPosTip(m_vtHoleLoc + m_vtPosMachPathOrigForCalc, vtPosServo);
...
// parameters
	...
	// machine span   = [1836.0  851.2]
	// machine center = [ 918.0  425.6]
	// align vacuum board center to machine center
	// start at (18.0  0.6)
	CVect2 vtExtentMin(0, 0);
	CVect2 vtExtentMax(1800, 850);
	...
	// z = 0 is bottom of stock
	double dThickness = 12.5;
	double dClearance = 4;
	double dHoleOverShoot = 0.5;
