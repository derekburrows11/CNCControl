...
void CPathMove::FindCurrentLimit()
{
	...
	for (;;)
	{
		if (m_pLimitList->GetNext(m_posLL) > ms1)
		{
			...
			if (m_TLimit >= 0)		// 'if' put back in 14/3/06. // sometimes limit was changed but position is just before limit start - so use limit just after ms1 position
			// not done now if m_TLimit < 0 14/3/06 //do 'GetPrev(m_posLL)' even if m_TLimit < 0 as will become positive with next advance
				m_pLimitList->GetPrev(m_posLL);
...
void CPathMove::GetTimeNextChange()
{
	...
	else				// next change is poly
	{
		...
		if (m_NextLimit.seg == GetSegNumber()+1 && m_NextLimit.s <= 0) //1e-12)	// next limit is at start of next segment - limit change too
